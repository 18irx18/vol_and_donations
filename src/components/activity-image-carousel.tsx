...
interface ActivityImagesCarouselProps {
  images?: string[];
}
...
export default function ActivityImagesCarousel({ images }: ActivityImagesCarouselProps) {
      ...
      {images.map((img, idx) => (
        <div key={idx} className="h-64 px-4">
          <div className="h-full w-full overflow-hidden rounded-md">
            <img src={img} alt={`Activity image ${idx + 1}`} className="w-full h-full object-cover" />
          </div>
        </div>
      ))}
    </Carousel>
